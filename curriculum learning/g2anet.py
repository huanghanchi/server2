...
class G2ANet(nn.Module):
    def __init__(self,  args):
        ...
        # Encoding
        self.encoding = nn.Linear(16, args.rnn_hidden_dim)  # 对所有agent的obs解码
    ...
    def forward(self, obs,hidden_state):
        ...
        obs=torch.cat((image_conv(obs[0]).reshape(obs[0].shape[0], -1),image_conv(obs[1]).reshape(obs[1].shape[0], -1)),0)
